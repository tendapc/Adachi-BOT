    ...
    if (skillres && skillres.skill_list) {
        ...
        if (avatar.actived_constellation_num >= 3) {
            ...
            if (info && info.constellations.length >= 5) {
                ...
                } else if (info.constellations[2].effect.includes("元素爆发")) {
                    skill.q.level_current += 3;
                }
                if (avatar.actived_constellation_num >= 5) {
                    if (info.constellations[4].effect.includes("元素战技")) {
                        skill.e.level_current += 3;
                    } else if (info.constellations[4].effect.includes("元素爆发")) {
                        skill.q.level_current += 3;
                    }
                }
            }
        ...
        for (var i = 0; i < data.avatars.length; i++) {
            ...
            if (avatar.rarity === 5) {
                if (avatar.id == 10000007 /*荧*/ || avatar.id == 10000005 /*空*/)
                    avatars[avatars.length] = c;
                else
                    fives[fives.length] = c;
            }
            else {
